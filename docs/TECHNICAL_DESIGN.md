...
当前智能体团队主要用于结构化协作和页面展示。模型任务本身由 `server/arkCore.js` 中的 Prompt 驱动，尚未引入外部 Agent 框架、MCP 工具调用或持久化任务队列。

### 可扩展方向

- 将模型 Provider 抽象成可配置接口。
- 增加工具调用层，例如岗位搜索 API、知识库检索和日志记录工具。
- 将智能体事件写入可持久化日志，支持评审查看任务执行轨迹。
...
- 当前不连接招聘平台账号，不自动投递。
- 当前不实现用户账号系统和数据库持久化。
- 公开提交日志前需要对姓名、手机号、邮箱、学校、岗位来源、API Key 和响应原文进行脱敏。
...
- 国产算力接入：增加 Gitee.AI/沐曦资源包 API 或沐曦算力卡调用路径。
- 日志审计：增加真实调用日志、延迟统计和失败率统计。
...
- 当前模型调用默认指向 Ark 兼容接口，未发现已完成的 Gitee.AI/沐曦专用适配。
- 当前没有真实模型调用日志样例。
- 当前没有真实性能测试数据。
- 当前没有演示视频链接和截图目录。
...
- 完成目标算力平台模型接入和真实调用验证。
- 补充脱敏运行日志和性能测试报告。
- 生成或上传演示视频、运行截图和评审查看路径。
- 如比赛平台强制要求，再通过 GitLink Issue 记录功能设计、模型调用、系统优化和技术难点。
