...
- Core Vision 与 Share Kit 已通过编译，但仍需要实际图片和分享动作的运行时复验。
- 核心业务页面仍为 ArkWeb 混合实现，不是全 ArkUI 原生页面。
- 没有生产数据库和跨设备同步；投递状态只保存在本机。
- Core Speech 尚未接入，语音识别和语音合成仍依赖浏览器兼容能力。
- Form Kit 服务卡片尚未实现。
...
- 使用真机补齐 Core Vision、Share Kit、华为账号和多设备布局验收。
- 接入 Core Speech 状态机和 Form Kit 今日行动卡片。
